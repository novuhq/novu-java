/* 
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
package co.novu.models.components;

import co.novu.utils.Utils;
import com.fasterxml.jackson.annotation.JsonCreator;
import com.fasterxml.jackson.annotation.JsonInclude.Include;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import jakarta.annotation.Nonnull;
import jakarta.annotation.Nullable;
import java.lang.Object;
import java.lang.Override;
import java.lang.String;
import java.util.List;
import java.util.Map;
import java.util.Optional;
import org.openapitools.jackson.nullable.JsonNullable;


public class SubscriberPayloadDto {
    /**
     * First name of the subscriber
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("firstName")
    private JsonNullable<String> firstName;

    /**
     * Last name of the subscriber
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("lastName")
    private JsonNullable<String> lastName;

    /**
     * Email address of the subscriber
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("email")
    private JsonNullable<String> email;

    /**
     * Phone number of the subscriber
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("phone")
    private JsonNullable<String> phone;

    /**
     * Avatar URL or identifier
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("avatar")
    private JsonNullable<String> avatar;

    /**
     * Locale of the subscriber
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("locale")
    private JsonNullable<String> locale;

    /**
     * Timezone of the subscriber
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("timezone")
    private JsonNullable<String> timezone;

    /**
     * Additional custom data associated with the subscriber
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("data")
    private JsonNullable<Map<String, Object>> data;

    /**
     * The internal identifier you used to create this subscriber, usually correlates to the id the user in
     * your systems
     */
    @JsonProperty("subscriberId")
    private String subscriberId;

    /**
     * An optional array of subscriber channels.
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("channels")
    private List<SubscriberChannelDto> channels;

    @JsonCreator
    public SubscriberPayloadDto(
            @JsonProperty("firstName") @Nullable JsonNullable<String> firstName,
            @JsonProperty("lastName") @Nullable JsonNullable<String> lastName,
            @JsonProperty("email") @Nullable JsonNullable<String> email,
            @JsonProperty("phone") @Nullable JsonNullable<String> phone,
            @JsonProperty("avatar") @Nullable JsonNullable<String> avatar,
            @JsonProperty("locale") @Nullable JsonNullable<String> locale,
            @JsonProperty("timezone") @Nullable JsonNullable<String> timezone,
            @JsonProperty("data") @Nullable JsonNullable<Map<String, Object>> data,
            @JsonProperty("subscriberId") @Nonnull String subscriberId,
            @JsonProperty("channels") @Nullable List<SubscriberChannelDto> channels) {
        this.firstName = Optional.ofNullable(firstName)
            .orElse(JsonNullable.undefined());
        this.lastName = Optional.ofNullable(lastName)
            .orElse(JsonNullable.undefined());
        this.email = Optional.ofNullable(email)
            .orElse(JsonNullable.undefined());
        this.phone = Optional.ofNullable(phone)
            .orElse(JsonNullable.undefined());
        this.avatar = Optional.ofNullable(avatar)
            .orElse(JsonNullable.undefined());
        this.locale = Optional.ofNullable(locale)
            .orElse(JsonNullable.undefined());
        this.timezone = Optional.ofNullable(timezone)
            .orElse(JsonNullable.undefined());
        this.data = Optional.ofNullable(data)
            .orElse(JsonNullable.undefined());
        this.subscriberId = Optional.ofNullable(subscriberId)
            .orElseThrow(() -> new IllegalArgumentException("subscriberId cannot be null"));
        this.channels = channels;
    }
    
    public SubscriberPayloadDto(
            @Nonnull String subscriberId) {
        this(null, null, null,
            null, null, null,
            null, null, subscriberId,
            null);
    }

    /**
     * First name of the subscriber
     */
    public JsonNullable<String> firstName() {
        return this.firstName;
    }

    /**
     * Last name of the subscriber
     */
    public JsonNullable<String> lastName() {
        return this.lastName;
    }

    /**
     * Email address of the subscriber
     */
    public JsonNullable<String> email() {
        return this.email;
    }

    /**
     * Phone number of the subscriber
     */
    public JsonNullable<String> phone() {
        return this.phone;
    }

    /**
     * Avatar URL or identifier
     */
    public JsonNullable<String> avatar() {
        return this.avatar;
    }

    /**
     * Locale of the subscriber
     */
    public JsonNullable<String> locale() {
        return this.locale;
    }

    /**
     * Timezone of the subscriber
     */
    public JsonNullable<String> timezone() {
        return this.timezone;
    }

    /**
     * Additional custom data associated with the subscriber
     */
    public JsonNullable<Map<String, Object>> data() {
        return this.data;
    }

    /**
     * The internal identifier you used to create this subscriber, usually correlates to the id the user in
     * your systems
     */
    public String subscriberId() {
        return this.subscriberId;
    }

    /**
     * An optional array of subscriber channels.
     */
    public Optional<List<SubscriberChannelDto>> channels() {
        return Optional.ofNullable(this.channels);
    }

    public static Builder builder() {
        return new Builder();
    }


    /**
     * First name of the subscriber
     */
    public SubscriberPayloadDto withFirstName(@Nullable String firstName) {
        this.firstName = JsonNullable.of(firstName);
        return this;
    }


    /**
     * Last name of the subscriber
     */
    public SubscriberPayloadDto withLastName(@Nullable String lastName) {
        this.lastName = JsonNullable.of(lastName);
        return this;
    }


    /**
     * Email address of the subscriber
     */
    public SubscriberPayloadDto withEmail(@Nullable String email) {
        this.email = JsonNullable.of(email);
        return this;
    }


    /**
     * Phone number of the subscriber
     */
    public SubscriberPayloadDto withPhone(@Nullable String phone) {
        this.phone = JsonNullable.of(phone);
        return this;
    }


    /**
     * Avatar URL or identifier
     */
    public SubscriberPayloadDto withAvatar(@Nullable String avatar) {
        this.avatar = JsonNullable.of(avatar);
        return this;
    }


    /**
     * Locale of the subscriber
     */
    public SubscriberPayloadDto withLocale(@Nullable String locale) {
        this.locale = JsonNullable.of(locale);
        return this;
    }


    /**
     * Timezone of the subscriber
     */
    public SubscriberPayloadDto withTimezone(@Nullable String timezone) {
        this.timezone = JsonNullable.of(timezone);
        return this;
    }


    /**
     * Additional custom data associated with the subscriber
     */
    public SubscriberPayloadDto withData(@Nullable Map<String, Object> data) {
        this.data = JsonNullable.of(data);
        return this;
    }


    /**
     * The internal identifier you used to create this subscriber, usually correlates to the id the user in
     * your systems
     */
    public SubscriberPayloadDto withSubscriberId(@Nonnull String subscriberId) {
        this.subscriberId = Utils.checkNotNull(subscriberId, "subscriberId");
        return this;
    }


    /**
     * An optional array of subscriber channels.
     */
    public SubscriberPayloadDto withChannels(@Nullable List<SubscriberChannelDto> channels) {
        this.channels = channels;
        return this;
    }


    @Override
    public boolean equals(java.lang.Object o) {
        if (this == o) {
            return true;
        }
        if (o == null || getClass() != o.getClass()) {
            return false;
        }
        SubscriberPayloadDto other = (SubscriberPayloadDto) o;
        return 
            Utils.enhancedDeepEquals(this.firstName, other.firstName) &&
            Utils.enhancedDeepEquals(this.lastName, other.lastName) &&
            Utils.enhancedDeepEquals(this.email, other.email) &&
            Utils.enhancedDeepEquals(this.phone, other.phone) &&
            Utils.enhancedDeepEquals(this.avatar, other.avatar) &&
            Utils.enhancedDeepEquals(this.locale, other.locale) &&
            Utils.enhancedDeepEquals(this.timezone, other.timezone) &&
            Utils.enhancedDeepEquals(this.data, other.data) &&
            Utils.enhancedDeepEquals(this.subscriberId, other.subscriberId) &&
            Utils.enhancedDeepEquals(this.channels, other.channels);
    }
    
    @Override
    public int hashCode() {
        return Utils.enhancedHash(
            firstName, lastName, email,
            phone, avatar, locale,
            timezone, data, subscriberId,
            channels);
    }
    
    @Override
    public String toString() {
        return Utils.toString(SubscriberPayloadDto.class,
                "firstName", firstName,
                "lastName", lastName,
                "email", email,
                "phone", phone,
                "avatar", avatar,
                "locale", locale,
                "timezone", timezone,
                "data", data,
                "subscriberId", subscriberId,
                "channels", channels);
    }

    @SuppressWarnings("UnusedReturnValue")
    public final static class Builder {

        private JsonNullable<String> firstName;

        private JsonNullable<String> lastName;

        private JsonNullable<String> email;

        private JsonNullable<String> phone;

        private JsonNullable<String> avatar;

        private JsonNullable<String> locale;

        private JsonNullable<String> timezone;

        private JsonNullable<Map<String, Object>> data;

        private String subscriberId;

        private List<SubscriberChannelDto> channels;

        private Builder() {
          // force use of static builder() method
        }

        /**
         * First name of the subscriber
         */
        public Builder firstName(@Nullable String firstName) {
            this.firstName = JsonNullable.of(firstName);
            return this;
        }

        /**
         * Last name of the subscriber
         */
        public Builder lastName(@Nullable String lastName) {
            this.lastName = JsonNullable.of(lastName);
            return this;
        }

        /**
         * Email address of the subscriber
         */
        public Builder email(@Nullable String email) {
            this.email = JsonNullable.of(email);
            return this;
        }

        /**
         * Phone number of the subscriber
         */
        public Builder phone(@Nullable String phone) {
            this.phone = JsonNullable.of(phone);
            return this;
        }

        /**
         * Avatar URL or identifier
         */
        public Builder avatar(@Nullable String avatar) {
            this.avatar = JsonNullable.of(avatar);
            return this;
        }

        /**
         * Locale of the subscriber
         */
        public Builder locale(@Nullable String locale) {
            this.locale = JsonNullable.of(locale);
            return this;
        }

        /**
         * Timezone of the subscriber
         */
        public Builder timezone(@Nullable String timezone) {
            this.timezone = JsonNullable.of(timezone);
            return this;
        }

        /**
         * Additional custom data associated with the subscriber
         */
        public Builder data(@Nullable Map<String, Object> data) {
            this.data = JsonNullable.of(data);
            return this;
        }

        /**
         * The internal identifier you used to create this subscriber, usually correlates to the id the user in
         * your systems
         */
        public Builder subscriberId(@Nonnull String subscriberId) {
            this.subscriberId = Utils.checkNotNull(subscriberId, "subscriberId");
            return this;
        }

        /**
         * An optional array of subscriber channels.
         */
        public Builder channels(@Nullable List<SubscriberChannelDto> channels) {
            this.channels = channels;
            return this;
        }

        public SubscriberPayloadDto build() {
            return new SubscriberPayloadDto(
                firstName, lastName, email,
                phone, avatar, locale,
                timezone, data, subscriberId,
                channels);
        }

    }
}
